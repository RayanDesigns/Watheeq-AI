...
export async function ensureFixtureClaims(): Promise<void> {
  ...
  const claims = [
    ...
    // Sprint-3 dedicated mutation claims so US-19 decisions don't race
    // against sprint-2 US-14 (which already mutates decide-*-claim-id).
    {
      id: "s3-decide-approve-claim-id",
      status: "under review",
      claimantID: CLAIMANT_UID,
      examinerID: EXAMINER_UID,
    },
    {
      id: "s3-decide-reject-claim-id",
      status: "under review",
      claimantID: CLAIMANT_UID,
      examinerID: EXAMINER_UID,
    },
    // Sprint-3 dedicated submitted claim picked by US-20 TC-S3-010 so the
    // pick action targets a known ID rather than "first card in the list".
    {
      id: "s3-pick-claim-id",
      status: "submitted",
      claimantID: CLAIMANT_UID,
    ...
    // Sprint-3 dedicated under-review claim with no AI fields — used by
    // US-22 TC-S3-016 to assert the streaming "loading" banner appears
    // immediately after navigation.
    {
      id: "s3-loading-claim-id",
      status: "under review",
      claimantID: CLAIMANT_UID,
      examinerID: EXAMINER_UID,
    },
...
/**
 * Fixed claim IDs used by the Sprint-3 AI draft tests (US-23). Each claim
 * is upserted with the four AI fields the live frontend reads when it
 * skips the streaming step and hydrates straight from the claim doc
 * (`if (claim.aiDecision) { … return; }` in the examiner detail page).
 *
 * Kept separate from `ensureFixtureClaims` so we don't conflate the
 * baseline under-review fixtures with the AI-analyzed ones.
 */
export const AI_COVERED_CLAIM_ID = "ai-covered-claim-id";
export const AI_NOT_COVERED_CLAIM_ID = "ai-not-covered-claim-id";

export const AI_COVERED_DRAFT = `Dear Seeded Patient,

We are pleased to inform you that your claim for Diagnostic Imaging has been reviewed and is approved under your Gold Health Plan. The requested treatment is covered as it falls within the eligible benefits described in your policy.

Should you have any questions, please contact our claims team.

Kind regards,
Watheeq Claims`;

export const AI_NOT_COVERED_DRAFT = `Dear Seeded Patient,

After careful review, we regret to inform you that the requested Diagnostic Imaging treatment is not covered under your current Gold Health Plan. The applicable policy clauses indicate this service is outside the scope of eligible benefits, and the claim has therefore been denied.

If you would like to discuss alternatives, please reach out to our support team.

Kind regards,
Watheeq Claims`;

export async function ensureAiSeededClaims(): Promise<void> {
  const app = getAdminApp();
  const db = app.firestore();
  const now = new Date();
  ...
  await db
    .collection("claims")
    .doc(AI_COVERED_CLAIM_ID)
    .set({
      ...baseClaim,
      claimId: AI_COVERED_CLAIM_ID,
      aiDecision: "covered",
      aiMessage:
        "The submitted Diagnostic Imaging service falls under the Gold Health Plan covered benefits, and all eligibility criteria are met.",
      aiDraft: AI_COVERED_DRAFT,
      aiDraftOriginal: AI_COVERED_DRAFT,
    });

  await db
    .collection("claims")
    .doc(AI_NOT_COVERED_CLAIM_ID)
    .set({
      ...baseClaim,
      claimId: AI_NOT_COVERED_CLAIM_ID,
      aiDecision: "not_covered",
      aiMessage:
        "The requested service is not covered under the applicable Gold Health Plan clauses.",
      aiDraft: AI_NOT_COVERED_DRAFT,
      aiDraftOriginal: AI_NOT_COVERED_DRAFT,
    });
}
