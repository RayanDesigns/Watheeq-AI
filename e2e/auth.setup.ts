...
setup.describe("Auth setup", () => {
  ...
  setup("seed Firestore fixtures", async () => {
    ...
    // Sprint-3 US-23 fixtures: claims with the AI fields pre-populated so
    // the examiner detail page hydrates directly from the doc and skips
    // the live Gemini stream — keeps the test deterministic and offline.
    await ensureAiSeededClaims();
  });
